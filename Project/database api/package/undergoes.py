...
class Undergoess(Resource):
    ...
    def post(self):
        ...
        conn.execute(
            ...
            (pat_id, proc_code, u_date, doc_id, nur_id, room_no))
        conn.commit()
        return undergoes
...
class Undergoes(Resource):
    ...
    def put(self, pat_id):
        ...
        room_no = undergoes['room_no']
        conn.execute("UPDATE undergoes SET doc_id=?,pat_id=?,proc_code=?,u_date=?,app_id=?,room_no=?, WHERE pat_id=?",
                     (doc_id, pat_id, proc_code, u_date, app_id, room_no, pat_id))
        conn.commit()
        return undergoes
